...
def match_pattern(initial_input_line: str, initial_pat: str):
    input_line = initial_input_line.removesuffix("\n")
    ...
    while True:
        def try_match():
            ...
            if len(pat) >= 2 and pat[0] == "\\" and pat[1].isdigit():
                target = ""
                pat = pat[1:]
                while pat and pat[0].isdigit():
                    target += pat[0]
                    pat = pat[1:]
                target = int(target) - 1  # Convert to zero-based index
                if target < len(cap_groups) and input_line.startswith(cap_groups[target]):
            ...
            if pat.startswith("\\d"):
                ...
                alt |= set(string.ascii_letters + string.digits + "_")
                pat = pat[2:]
            elif pat.startswith("[^"):
                positive = False
                end = pat.find("]", 2)
            ...
            n_matches = 0
            while len(input_line) > 0 and (anything or (input_line[0] in alt) == positive):
